...
fun solve(lines: List<String>): Int {
    ...
    for ((i, cube) in sortedCubesInTetris.withIndex()) {
        ...
        val removedCubes = mutableListOf<Cube>()
        for (j in i + 1 until sortedCubesInTetris.size) {
            val cube2 = sortedCubesInTetris[j]
            tetris.removeCube(cube2)
            if (tetris.canFitCube(cube2.shift(shiftDown))) {
                removedCubes.add(cube2)
            } else {
                tetris.addCube(cube2)
            }
        }
        result += removedCubes.size
        for (cube2 in removedCubes) {
            tetris.addCube(cube2)
        }
        tetris.addCube(cube)
    }
